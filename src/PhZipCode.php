...
class PhZipCode
{
    ...
    public function getZipCodes()
    {
        return array_map(fn ($region) => [
            ...
            'name' => u($region['name']),
            'provinces' => array_map(fn ($province) => [
                'region' => u($region['region']),
    ...
    {
        return array_merge(
            ...array_map(fn ($region) => array_map(fn ($province) => [
    ...
    public function getMunicipalities($region, $province)
    {
        if ($provinces = $this->getProvinces($region)) {
            return array_merge(
                array_filter($provinces, fn ($item) => u($item['province']) === u($province))
            );
        }
    ...
    public function getBarangays($region, $province, $municipality)
    {
        if ($provinces = $this->getMunicipalities($region, $province)) {
            return array_merge(
                ...array_map(
                    fn ($municipality) => $municipality['barangays'],
                    array_filter(
                        $this->getZipCodesList(true),
                        fn ($item) => u($item['municipality']) === u($municipality)
                    )
                )
            );
        }
    ...
    {
        return array_merge(
            ...array_filter($this->getZipCodesList(true), fn ($item) => u($item['zip_code']) === u($zipCode)) ?: []
        );
    }
}
